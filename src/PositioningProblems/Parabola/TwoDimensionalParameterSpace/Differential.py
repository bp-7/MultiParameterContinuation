...
def systemEvaluation(S, h):
    R, r, t, mu = S[0], S[1], S[2], S[3]
    phi = matrixRepresentationOfSE3Element(R, r)

    u = phi @ Parabola(t[0], mu[1]) @ rho_z(0.5 * np.pi - mu[0]) @ tau_x(0.5 * h)
    v = phi @ Parabola(t[1], mu[1]) @ rho_z(- S[2][2]) @ tau_x(-0.5 * h)

    return [u[:3, :3], u[:3, 3], v[:3, :3], v[:3, 3]]

def E1(t1, mu, h):
    return Parabola(t1, mu[1]) @ rho_z(-mu[0] + 0.5 * np.pi) @ tau_x(0.5 * h)

def E2(t2, t3, mu, h):
    return Parabola(t2, mu[1]) @ rho_z(-t3) @ tau_x(-0.5 * h)
...
def differentialRnF(S, v, h):
    R, r, t, mu = S[0], S[1], S[2], S[3]
    phi = matrixRepresentationOfSE3Element(R, r)
    d0 = v[0] * phi @ ParabolaDerivative(t[0], mu[1]) @ rho_z(-mu[0] + 0.5 * np.pi) @ tau_x(0.5 * h)
    d1 = v[1] * phi @ ParabolaDerivative(t[1], mu[1]) @ rho_z(-t[2]) @ tau_x(-0.5 * h) \
         - v[2] * phi @ Parabola(t[1], mu[1]) @ rho_z(-t[2]) @ se3Basis.G3 @ tau_x(-0.5 * h)

    return [d0[:3, :3], d0[:3, 3], d1[:3, :3], d1[:3, 3]]

def differentialSolution(S, zeta, h):
    R, r, t, mu = S[0], S[1], S[2], S[3]
    xi = np.concatenate((InvSkew(zeta[0]), zeta[1]))
    E_1, E_2 = E1(t[0], mu, h), E2(t[1], t[2], mu, h)

    d0 = differentialSE3F(E_1, E_2, xi)
    d1 = differentialRnF(S, zeta[2], h)

    return [d0[i] + d1[i] for i in range(len(d0))]

def differentialParameter(S, v, h):
    R, r, t, mu = S[0], S[1], S[2], S[3]
    phi = matrixRepresentationOfSE3Element(R, r)

    d0 = v[1] * phi @ ParabolaParameterDerivative(t[0], mu[1]) @ rho_z(-mu[0] + 0.5 * np.pi) @ tau_x(0.5 * h)  \
         - v[0] * phi @ Parabola(t[0], mu[1]) @ rho_z(-mu[0] + 0.5 * np.pi) @ se3Basis.G3 @ tau_x(0.5 * h)

    d1 = v[1] * phi @ ParabolaParameterDerivative(t[1], mu[1]) @ rho_z(-t[2]) @ tau_x(-0.5 * h)
